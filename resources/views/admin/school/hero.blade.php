        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $schoolHero->title ?? '') }}" class="input-field" placeholder="Enter hero title">
                    @error('title')
                ...
                <div>
                    ...
                    <textarea name="subtitle" class="textarea-field" rows="3" placeholder="Enter hero subtitle">{{ old('subtitle', $schoolHero->subtitle ?? '') }}</textarea>
                    @error('subtitle')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="title_font_size" class="input-field">
                        @foreach([24,28,32,36,40,48] as $size)
                            <option value="{{ $size }}" {{ old('title_font_size', $schoolHero->title_font_size ?? '') == $size ? 'selected' : '' }}>{{ $size }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="subtitle_font_size" class="input-field">
                        @foreach([14,16,18,20,24] as $size)
                            <option value="{{ $size }}" {{ old('subtitle_font_size', $schoolHero->subtitle_font_size ?? '') == $size ? 'selected' : '' }}>{{ $size }}</option>
                        @endforeach
                    </select>
            ...
            <div class="image-upload-area border-2 border-dashed border-gray-300 rounded-lg p-6 cursor-pointer hover:border-blue-500 transition-colors" onclick="document.getElementById('hero-image').click()">
                <div id="upload-placeholder" class="text-center">
                    ...
                    <p class="text-gray-500">
                        @if(isset($schoolHero) && $schoolHero->cover_image)
                ...
                <img 
                    id="image-preview" 
                    class="max-w-full max-h-32 rounded-lg mx-auto mt-4 {{ isset($schoolHero) && $schoolHero->cover_image ? '' : 'hidden' }}" 
                    src="{{ isset($schoolHero) && $schoolHero->cover_image ? asset('storage/' . $schoolHero->cover_image) : '' }}" 
                    alt="Image Preview"
                />
            </div>
